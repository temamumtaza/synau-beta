...
- ✅ **N+1 Convex queries resolved** — Dashboard / course-detail / lesson pages were each firing 4–32 sequential HTTP round-trips to the Convex gateway (course → chapters → lessons, with `cache: 'no-store'`), making navigation take 5–10s. Replaced with three aggregated Convex queries — `courses:listWithStats`, `courses:getFullDetail`, `lessons:getLessonPage` — that collapse each page to a single round-trip (dashboard: ~32 → 1). `convexQuery` now supports opt-in Next.js caching (`{ revalidate, tags }`); dashboard & course-detail read with a 10s revalidate for instant back/forward, while the lesson page stays fresh (status is time-sensitive). `loading.tsx` skeletons added at the dashboard / course / lesson segments so the shell streams instantly. Remaining lever: migrate reads to the Convex React client (`useQuery`) for true real-time + zero round-trips on revisits (see known issue #9).
...
- **Performance — page navigation overhaul** — Fixed the slow page transitions / rendering. Root cause: every read-heavy Server Component fired N+1 sequential HTTP round-trips to the Convex gateway with `cache: 'no-store'` (dashboard: user → courses → per-course progress + chapters → per-chapter lessons ≈ 32 round-trips; lesson page: 4–8 sequential incl. a per-chapter prev/next scan). Added three aggregated Convex queries that collapse each page to a **single** round-trip: `courses:listWithStats(userId)` (courses + lesson stats + last activity), `courses:getFullDetail(courseId)` (course + roadmap + chapters + nested lessons + counts), `lessons:getLessonPage(lessonId)` (lesson + course + chapter + content + citations + prev/next). Key trick: leveraged the existing `lessons.by_course` index to fetch all lessons of a course in one query instead of looping per chapter. `convexQuery` now takes an optional `{ revalidate, tags }` for Next.js data caching — dashboard & course-detail use `revalidate: 10` for instant back/forward navigation; lesson page stays uncached (lesson status is time-sensitive). Added `loading.tsx` skeletons at the `(dashboard)/dashboard`, `[courseId]`, and `[lessonId]` segments so the app shell renders immediately on navigation. `tsc` + `lint` clean.
